...
export function ProjectsView({ category }: { category: string }) {
  ...
  return (
    <><PageHero variant="projects" /><section className="projects-index container">
      ...
      <nav className="filter-nav" aria-label="프로젝트 분류">{categories.map((item) => <Link key={item.slug} href={`/projects/${item.slug}`} scroll={false} aria-current={category === item.slug ? "page" : undefined}>{item.label}</Link>)}</nav>
