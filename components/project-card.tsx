  ...
  return (
    <article className="project-card">
      ...
      <Link href={`/projects/detail/${project.slug}`} className="project-visual" aria-label={`${project.title} 프로젝트 상세 보기`} style={{ "--project-accent": project.accent } as React.CSSProperties}>
        {project.image ? <Image className="project-card-image" src={project.image} alt={`${project.title} 대표 화면`} width={516} height={400} sizes="(max-width: 640px) 342px, 516px" priority={priority} /> : null}
      </Link>
