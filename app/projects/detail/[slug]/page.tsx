...
function StandardDetail({ project }: { project: (typeof projects)[number] }) {
  return <><section className="detail-section container"><Reveal><span className="eyebrow">Background &amp; Goal</span><h2>기획배경 &amp; 문제정의</h2><div className="detail-cards"><article><span>Background</span><h3>복잡한 정보를 이해하기 쉬운 경험으로</h3><p>{project.summary}</p></article><article><span>Project Goal</span><h3>탐색 효율과 서비스 신뢰도 개선</h3><p>사용자의 핵심 과업을 중심으로 정보 구조와 화면 위계를 재정의하고 일관된 경험을 설계했습니다.</p></article></div></Reveal></section><section className="detail-section container"><Reveal><span className="eyebrow">Design</span><h2>주요화면</h2><div className="detail-showcase detail-showcase-image" style={{ "--project-accent": project.accent } as React.CSSProperties}><div className="detail-project-image"><Image src={`/assets/detail/${project.slug}-hero.png`} alt={`${project.title} 주요 화면`} width={1056} height={640} sizes="(max-width: 640px) calc(100vw - 48px), 1056px" /></div><div><h3>핵심 과업 중심 화면 설계</h3><p>반응형 구조와 명확한 정보 위계를 적용해 빠른 탐색과 깊이 있는 검토를 함께 지원합니다.</p></div></div></Reveal></section></>;
}
